...
const winterCollection = (req, res) => {
  res.json(winterCollectionData);
};
const winterJackets = (req, res) => {
  let jacket = winterCollectionData.filter((item) => item.category === "Jackets");
  res.json({ array: jacket });
};
const winterShirt=(req,res)=>{
let shirts=winterCollectionData.filter((item)=>item.category==="winterShirt");
res.json({array:shirts})
}
const winterPants=(req,res)=>{
  let pants=winterCollectionData.filter((item)=>item.category==="winterPants");
  res.json({array:pants})
  }
module.exports = {
  winterCollection,
  winterJackets,
